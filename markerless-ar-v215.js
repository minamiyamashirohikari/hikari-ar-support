(function () {
  ...
  // Safari's user agent cannot distinguish an iPad mini 4 from every newer
  // iPad reliably. Keep the low-memory 3D camera route opt-in, and let the
  // platform capability decide the normal iPad route.
  const explicitLegacy3dRequested = isIPad
    && new URLSearchParams(location.search).get('legacy3d') === '1';
  const needsLegacyIPadCamera = explicitLegacy3dRequested;
  ...
  const SPATIAL_RUNTIME_URLS = [
    'vendor/8thwall/xr.js?v=20260819-ipad42',
  ...
  let nativeArObjectPlaced = false;
  let nativeArFallbackScheduled = false;
  let spatialArOpening = false;
  let spatialRuntimePreparation = null;
  let spatialRuntimePreparationSettled = !(isAndroid || (isIPad && !needsLegacyIPadCamera));
  let spatialRuntimePreparationFailed = false;
  ...
  const legacy3dRequested = explicitLegacy3dRequested;
  ...
  function beginNativeArAttempt() {
    ...
    nativeArObjectPlaced = false;
    nativeArFallbackScheduled = false;
    // Preserve the existing iPhone timeout. On iPad the attempt must remain
    // active until placement or exit, otherwise a late Quick Look failure can
    // no longer recover to the shared spatial route.
    if (!isIPad) nativeArAttemptTimer = window.setTimeout(clearNativeArAttempt, 30000);
  }

  function scheduleNativeArFallback(code = 'NATIVE_AR_FAILED', reason = '端末標準ARで料理を配置できませんでした。') {
    if (nativeArFallbackScheduled || spatialArOpening) return;
    nativeArFallbackScheduled = true;
    trialLog?.markFailure(reason, { stage: 'native_ar', code });
    clearNativeArAttempt();
    window.setTimeout(openNativeArFallback, 0);
  }

  async function prepareSpatialRuntime() {
    if (!(isAndroid || (isIPad && !needsLegacyIPadCamera))) return true;
    if (spatialRuntimePreparation) return spatialRuntimePreparation;
    spatialRuntimePreparationSettled = false;
    spatialRuntimePreparationFailed = false;
    updateArAvailability();
    spatialRuntimePreparation = (async () => {
      for (const path of SPATIAL_RUNTIME_URLS) {
        ...
        try {
          ...
          if (!response.ok) throw new Error(`Spatial runtime unavailable: ${path}`);
      ...
      // The spatial page retries these resources itself. A failed warm-up must
      // not leave the AR button permanently disabled.
      spatialRuntimePreparationFailed = true;
      return false;
    }).finally(() => {
      spatialRuntimePreparationSettled = true;
      updateArAvailability();
    });
    return spatialRuntimePreparation;
  }
  ...
  async function loadLocalPair(revision) {
    ...
    try {
      if (needsLegacyIPadCamera) {
        leftBytes = await modelBytes(leftId);
        if (revision !== pairLoadRevision) return;
        rightBytes = await modelBytes(rightId);
      } else {
      ...
      if (isAndroid || (isIPad && !needsLegacyIPadCamera)) prepareSpatialRuntime();
      ...
      replaceViewerSource(byId.get(leftId).modelUrl, false, revision);
      setMessage('2品の準備に失敗したため、候補1を安全表示しています。', 'warning');
    } finally {
      if (needsLegacyIPadCamera) {
        modelBufferCache.delete(leftId);
        modelBufferCache.delete(rightId);
      }
      leftBytes = null;
      rightBytes = null;
    }
  ...
  function filteredItems() {
    ...
    return items.filter((item) => {
      // A typed search always covers every available dish. Restricting it to the
  ...
  function updateArAvailability() {
    ...
    if (isAndroid && !spatialRuntimePreparationSettled) {
      nativeArButton.hidden = true;
      browserArButton.disabled = true;
      simpleCameraArButton.disabled = false;
      browserArButton.textContent = '空間ARを準備しています';
      deviceNote.textContent = '選んだ料理とAR機能を端末へ準備しています。完了すると起動できます。';
      return;
    }
    browserArButton.disabled = false;
    simpleCameraArButton.disabled = !pairModelReady;
    nativeArButton.hidden = true;
    if (isIPad && nativeArSupported) {
      browserArButton.textContent = 'iPad標準ARを起動';
      deviceNote.textContent = 'iPhoneと同じApple標準ARを使用します。アプリ独自の机認識は行いません。';
    } else if (isIPad) {
      browserArButton.textContent = '共通空間ARを起動';
      deviceNote.textContent = 'Apple標準ARを利用できないため、共通空間ARへ切り替えます。';
    } else if (isAndroid) {
      browserArButton.textContent = '空間ARを起動';
      deviceNote.textContent = spatialRuntimePreparationFailed
        ? '事前準備を完了できなかったため、起動後にAR機能を再取得します。'
        : '料理とAR機能を準備済みです。料理を表示してから机へ固定します。';
  ...
  function beginTrial(route) {
    return trialLog?.begin(route, selected, false);
  }
  ...
  function openPaperComparison() {
    trialLog?.markPaperSwitch();
    location.href = paperComparisonUrl().href;
  }
  ...
  function openSpatialAr() {
    ...
    if (isAndroid && !spatialRuntimePreparationSettled) {
      setMessage('共通空間ARを準備しています。準備完了後にもう一度押してください。', 'warning');
      prepareSpatialRuntime();
      return;
    }
  ...
  function openNativeArFallback() {
    if (spatialArOpening) return;
  ...
  async function openPreferredAr() {
    // Legacy iPads cannot run Apple AR. Modern iPads use the same native
    // Quick Look route as iPhone, while Android keeps the shared WebAR route.
    if (needsLegacyIPadCamera) {
      if (!pairModelReady) {
        setMessage('高品質な2品の3Dを準備しています。準備完了後にもう一度押してください。', 'warning');
        return;
      }
      beginTrial('旧iPad 3Dカメラ比較');
      await openCameraAr();
      return;
    }
    if (isAndroid) {
      beginTrial('共通空間AR');
      trialLog?.markStage('route_transition', 'SHARED_AR_OPENING');
      openSpatialAr();
      return;
    }
    if (isIPad && !nativeArSupported) {
    ...
    browserArButton.disabled = true;
    beginTrial(`${isIPad ? 'iPad' : 'iPhone'}標準AR`);
    trialLog?.markStage('native_ar', 'NATIVE_AR_STARTING');
    setMessage(`${isIPad ? 'iPad' : 'iPhone'}標準ARを起動しています`);
    beginNativeArAttempt();
    try {
      await viewer.activateAR();
    } catch (_) {
      scheduleNativeArFallback('NATIVE_ACTIVATION_REJECTED', '端末標準ARの起動要求が失敗しました。');
    } finally {
      browserArButton.disabled = false;
    }
  }
  ...
  function updateCameraReadyStatus() {
    if (cameraVideoReady && cameraModelReady) {
      ...
      trialLog?.markDisplay({ stage: 'camera_preview', code: 'CAMERA_3D_VISIBLE' });
  ...
  async function startCameraFeed() {
    ...
    trialLog?.markStage('camera_stream', 'CAMERA_STREAM_RECEIVED');
  ...
  async function startCameraForOverlay() {
    ...
    try {
      ...
      trialLog?.markFailure(cameraFailureMessage(error), {
        stage: error?.name === 'NotAllowedError' || error?.name === 'SecurityError'
          ? 'camera_permission'
          : 'camera_stream',
        code: error?.name === 'CameraTimeoutError' ? 'CAMERA_PERMISSION_TIMEOUT' : 'CAMERA_START_FAILED'
      });
      retry.hidden = false;
    } finally {
      cameraOpening = false;
  ...
  async function openCameraAr() {
    ...
    trialLog?.transition(needsLegacyIPadCamera ? '旧iPad 3Dカメラ比較' : '3Dカメラ比較', selected);
    const left = byId.get(selected[0]);
    const right = byId.get(selected[1]);
  ...
  function handleNativeArStatus(event) {
    ...
    if (messages[status]) setMessage(messages[status], status === 'failed' ? 'warning' : 'normal');

    if (status === 'session-started') {
      // iPad is not counted as successful until the dish is actually placed.
      // iPhone keeps its established session-start success definition.
      if (!isIPad) {
        trialLog?.markDisplay({ stage: 'native_ar_session', code: 'NATIVE_SESSION_STARTED' });
        clearNativeArAttempt();
      } else {
        trialLog?.markStage('native_ar_session', 'NATIVE_SESSION_STARTED');
      }
      return;
    }
    if (status === 'object-placed') {
      nativeArObjectPlaced = true;
      trialLog?.markDisplay({ stage: 'native_ar_placement', code: 'OBJECT_PLACED' });
      clearNativeArAttempt();
      return;
    }
    if (status === 'failed') {
      if (nativeArAttempted && (!isIPad || !nativeArObjectPlaced)) {
        scheduleNativeArFallback('NATIVE_STATUS_FAILED', '端末標準ARが失敗を通知しました。');
      }
      return;
    }
    if (status === 'not-presenting') {
      if (isIPad && nativeArAttempted && !nativeArObjectPlaced) {
        scheduleNativeArFallback('NATIVE_NOT_PLACED', '端末標準ARを料理配置前に終了しました。');
      }
      else clearNativeArAttempt();
    }
  }

  viewer.addEventListener('ar-status', handleNativeArStatus);
  ...
  cameraArViewer.addEventListener('error', () => {
    ...
    trialLog?.markFailure('料理の立体を読み込めませんでした。', {
      stage: 'model_load',
      code: 'CAMERA_MODEL_LOAD_FAILED'
    });
  });
  ...
  document.getElementById('paperComparisonButton').addEventListener('click', openPaperComparison);
  ...
  document.getElementById('retryCameraArButton').addEventListener('click', () => {
    trialLog?.markRestart();
    startCameraForOverlay();
  });
  document.getElementById('paperCameraArButton').addEventListener('click', openPaperComparison);
  ...
  function refreshTrialSummary(message = '') {
    const summary = trialLog?.summary() || {};
    document.getElementById('trialAttempts').textContent = `${summary.attempts || 0}回`;
    document.getElementById('trialSuccessRate').textContent = percentage(summary.successRate);
    document.getElementById('trialMedianTime').textContent = Number.isFinite(summary.medianDisplayMs)
      ? `${(summary.medianDisplayMs / 1000).toFixed(1)}秒`
      : '—';
    document.getElementById('trialAssistRate').textContent = percentage(summary.assistRate);
    document.getElementById('trialGuidance').textContent = message
      || '表示成功率95%以上・表示中央値3秒以内・職員介助10%以下は、本試験へ進むための運用上の目安です。';
  }
  document.getElementById('trialLogButton').addEventListener('click', () => {
    refreshTrialSummary();
    trialDialog.showModal();
  });
  document.getElementById('closeTrialButton').addEventListener('click', () => trialDialog.close());
  trialDialog.addEventListener('click', (event) => {
    if (event.target === trialDialog) trialDialog.close();
  });
  document.getElementById('recordAssistButton').addEventListener('click', () => {
    const recorded = trialLog?.markStaffAssist();
    refreshTrialSummary(recorded
      ? '直前の試行に職員介助ありを記録しました。'
      : '記録できるAR試行がありません。先にARを起動してください。');
  });
  document.getElementById('exportTrialButton').addEventListener('click', () => {
    trialLog?.downloadCsv();
    refreshTrialSummary('CSVを書き出しました。記録はこの端末内にも残っています。');
  });
