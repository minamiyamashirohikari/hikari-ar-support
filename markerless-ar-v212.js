(function () {
  ...
  function updateArAvailability() {
    ...
    if (isIPad && nativeArSupported) {
      ...
      browserArButton.textContent = '3Dカメラ比較を起動';
      deviceNote.textContent = 'Apple標準ARを利用できないため、高品質な2品をカメラ映像の上に3D表示します。';
    } else if (isAndroid) {
  ...
  function openNativeArFallback() {
    if (isIPad) {
      setMessage('Apple標準ARを開始できなかったため、3Dカメラ比較へ切り替えます。', 'warning');
      void openCameraAr();
      return;
    }
  ...
  viewer.addEventListener('ar-status', (event) => {
    const messages = {
      ...
      failed: isIPad
        ? '標準ARを起動できなかったため、3Dカメラ比較へ切り替えます。'
        : '標準ARを起動できなかったため、共通空間ARへ切り替えます。',
